...
async def onCallbackQuery(client, query):
    ...
    if qdata:
        ...
        if re.search(r"prev#", raw_data) or re.search(r"next#", raw_data):
            ...
            if offset < 0:
                await query.answer("No more users prev")
                return
            elif offset > len(await db.getUsers()):
                await query.answer("No more users next")
                return
            await query.answer()
            # if raw_data.startswith("prev"):
            #     offset -= limit
            # else:
            #     offset += limit
            back = offset - limit
            ...
            for datas in userspage:
                ...
                listUser += f"{status} <a href='https://t.me/{client.username}?start=login-{datas['idtele']}'>{datas['idtele']}</a><i> {uname}  -  {life_logged}</i>\n"
            if listUser != "":
        ...
        if sesi_cb[0] == "delsesi":
            ...
            await np.stop()
            await message.reply_text("Session berhasil dihapus")
            return
        ...
        if data == "clearchat":
            ...
            send_clear = await message.reply_text("Clearing chats...")
            np = NpClient(user=usr, db=db)
            await np.init(client, message)
            clear = await np.clearChats()
            await np.stop()
            if clear:
                await send_clear.edit_text("Chat cleared")
            else:
                await send_clear.edit_text("Failed to clear chats")
            return
        if data == "changenohp":
            usr = await db.getUser(idacc)
            if usr is None:
            ...
            try:
                newphone = await message.chat.ask(
                    "Kirim nomer hp barunya",
                    filters=filters.text,
                    timeout=60,
                )
                if "/cancel" in newphone.text:
                    await message.reply_text("Kenapa dibatalin")
                    return
            except BaseException as e:
                asu = str(e)
                if asu == "60":
                    await message.reply_text("Waktu habis, lama banget")
                    return
                print(e)
        ...
        if data == "listsesi":
            ...
            await np.stop()
            if sessions is None:
                await message.reply_text("Failed to get sessions")
                return
            sendlist = await message.reply(
                text=f"List session `{usr['idtele']}`",
        ...
        if data == "readcode":
            ...
            if initUser is not None:
                ...
                await message.reply_text("Account failed to initialize")
                return
